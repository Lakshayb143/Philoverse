...
async def reset_conversation_state() -> dict:
    """
    Deletes all the conversation state data from mongodb
    ...
    try:
        ...
        if settings.MONGO_STATE_WRITE_COLLECTION in db.list_collection_names():
            db.drop_collection(settings.MONGO_STATE_WRITE_COLLECTION)
            collections_deleted.append(settings.MONGO_STATE_WRITE_COLLECTION)
            logger.info(
